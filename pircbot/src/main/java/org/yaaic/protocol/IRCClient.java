...
public abstract class IRCClient {
    ...
    public final synchronized void connect(String hostname) throws IOException, IrcException, NickAlreadyInUseException {
        this.connect("irc.marocchat.nl", 6667, null);
    }
    ...
    public final synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        if (_useSSL) {
            try {
                ...
                SSLSocket ssocket = (SSLSocket) factory.createSocket("irc.marocchat.nl", 6667);
    ...
    private void setSNIHost(final SSLSocketFactory factory, final SSLSocket socket, final String hostname) {
        if (factory instanceof android.net.SSLCertificateSocketFactory && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ((android.net.SSLCertificateSocketFactory)factory).setHostname(socket, "irc.marocchat.nl");
        } else {
            try {
                socket.getClass().getMethod("setHostname", String.class).invoke(socket, "irc.marocchat.nl");
            } catch (Throwable e) {
    ...
    public final void ignore(String channel, String nick) {
        this.sendRawLine("ignore " + nick);
    }

    public final void unignore(String channel, String nick) {
        this.sendRawLine("ignore -r " + nick);
    }
    ...
    public final void kick(String channel, String nick) {
        this.kick(channel, nick, "Why Are you Tease Me Thats Why I Kick You :p");
    }
